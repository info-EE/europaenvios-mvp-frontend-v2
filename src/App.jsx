/*  Europa Envios – MVP 0.2.5 (COMPLETO)
   - Armado de cajas robusto (sin pantalla blanca), caja activa + Editar/Guardar, reordenar y eliminar.
   - Impresion via iframe oculto (evita about:blank) y sin acentos en etiquetas.
   - Recepcion con estados restringidos por prefijo de carga (AIR/MAR/COMP).
   - Gestion de cargas: alerta si faltan paquetes sin asignar al pasar a En transito/Arribado.
   - Exportaciones XLSX con estilos y plantillas en /public/templates.
   - Proformas: mantiene colores de la plantilla, fecha de carga y courier, cantidades a 3 decimales, precios a 2,
     5 filas reservadas para EXTRAS debajo de la comision 4%, TOTAL al final. SIN usar formula IMAGE/IMAGEN.
*/
...
/* Reemplazo de placeholders de TEXTO (no toca imagenes) */
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((it, i) => (
          <span key={i} className="px-2 py-1 bg-white border rounded">
            {it}{" "}
            <button onClick={() => del(i)} className="text-red-600">×</button>
          </span>
        ))}
      </div>
    </div>
...
function Reception({ currentUser, couriers, setCouriers, estados, setEstados, flights, onAdd }) {
  ...
  const ok = () =>
    [
      "courier","estado","casilla","codigo","fecha","empresa","nombre","tracking","remitente",
      "peso_real_txt","L_txt","A_txt","H_txt","desc","valor_txt",
  ...
  const submit = () => {
    ...
    setForm({
      ...form,
      casilla: "", codigo: "", empresa: "", nombre: "", tracking: "", remitente: "",
      peso_real_txt: "", L_txt: "", A_txt: "", H_txt: "", desc: "", valor_txt: "0,00", foto: null,
    });
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        alert("No se pudo acceder a la camara."); setCamOpen(false);
      }
    })();
    return () => {
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow w-full max-w-3xl">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button className={BTN} onClick={() => setCamOpen(false)}>Cerrar</button>
            </div>
...
function PaquetesBodega({ packages, flights, user, onUpdate }) {
  ...
  async function exportXLSX() {
    const header = [
      th("Carga"), th("Courier"), th("Estado"), th("Casilla"), th("Codigo de paquete"), th("Fecha"),
      th("Empresa de envio"), th("Nombre y apellido"), th("Tracking"), th("Remitente"),
      th("Peso facturable (min 0,200 kg)"), th("Exceso de volumen"), th("Medidas"), th("Descripcion"), th("Precio (EUR)"),
    ];
    const body = rows.map((p) => {
      ...
      return [
        ...
        td(fmtPeso(p.peso_facturable)), td(fmtPeso(p.exceso_volumen)), td(medidas), td(p.descripcion || ""), td(fmt2(p.valor_aerolinea || 0)),
      ];
    });
    ...
    >
      <div className="overflow-auto">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              {["Carga","Codigo","Casilla","Fecha","Nombre","Tracking","Peso real","Medidas","Exceso de volumen","Descripcion","Foto","Editar"].map((h) => (
                <th key={h} className="text-left px-3 py-2">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={12} className="text-center text-gray-500 py-6">No hay paquetes.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow w-full max-w-3xl">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button className={BTN} onClick={() => setViewer(null)}>Cerrar</button>
            </div>
            <div className="p-4">
  ...
  setFlights(
    flights.map((f) =>
      f.id !== flightId
        ...
        : {
            ...f,
            cajas: cajas.map((c) => (c.id !== activeBoxId ? c : { ...c, paquetes: [...c.paquetes, pkg.id] })),
          }
    )
  );
  onAssign?.(pkg.id);
  ...
  setFlights((prev) =>
    prev.map((f) =>
      f.id !== flightId
        ...
        : {
            ...f,
            cajas: f.cajas.map((c) => (c.id === fromId ? { ...c, paquetes: c.paquetes.filter((x) => x !== pid) } : c)),
          }
    )
  );
  setFlights((prev) =>
    prev.map((f) =>
      f.id !== flightId
        ...
        : {
            ...f,
            cajas: f.cajas.map((c) => (c.id === toId ? { ...c, paquetes: [...c.paquetes, pid] } : c)),
          }
    )
  );
}
          ...
          cajas.map((c) => {
            ...
            return (
              <div
                ...
                className={
                  "border rounded-2xl p-3 mb-3 hover:ring-2 hover:ring-indigo-300 cursor-pointer " +
...
function CargasEnviadas({ packages, flights }) {
  ...
  const list = flights
    .filter((f) => f.estado !== "En bodega")
    .filter((f) => (!from || f.fecha_salida >= from) && (!to || f.fecha_salida <= to) && (!estado || f.estado === estado));
  ...
  async function exportTodo() {
    ...
    if (tpl) {
      ...
      appendSheet(
        ...
        "CAJAS",
        [
          [th("Nº Caja"), th("Courier"), th("Peso"), th("Largo"), th("Ancho"), th("Alto"), th("Volumetrico")],
...
/* ===== Proformas (mantiene colores plantilla; sin IMAGE/IMAGEN) ===== */
...
function Proformas({ packages, flights, extras }) {
  ...
  function writeFacturaSheet(wb, r) {
    ...
    // FECHA y COURIER
    ws["A9"] = { v: flight?.fecha_salida || new Date().toISOString().slice(0, 10), t: "s", s: ws["A9"]?.s || {} };
    ws["A12"] = { v: r.courier, t: "s", s: ws["A12"]?.s || {} };

    // Inicio del detalle
    const start = 16;
    const slotsReservados = 5;
    const fmtQty = "#,##0.000";
    const fmtMoney = "#,##0.00";
    ...
    const listExtras = extrasDeCourier(r.courier);
    const montoExtras = listExtras.reduce((s, e) => s + parseComma(e.monto), 0);
    const comision4 = 0.04 * (proc + fr + fe + montoExtras);
    ...
    let lines = [
      ...
      ["Comision por transferencia (4%)", 0, 0, comision4],
      ...Array.from({ length: slotsReservados }, () => ["", "", "", ""]),
    ];

    listExtras.forEach((e) => {
    ...
    lines.forEach((ln, idx) => {
      ...
      ws[`D${row}`] = { v: typeof tot === "number" ? tot : tot || "", t: "n", z: fmtMoney, s: ws[`D${row}`]?.s || {} };
    });

    const totalNum = lines.reduce((s, ln) => s + (Number(ln[3]) || 0), 0);
    const totalRow = start + lines.length;
    ws[`A${totalRow}`] = { v: "TOTAL USD", t: "s", s: ws[`A${totalRow}`]?.s || {} };
    ws[`D${totalRow}`] = { v: totalNum, t: "n", z: fmtMoney, s: ws[`D${totalRow}`]?.s || {} };
  }
  ...
  async function exportX(r) {
    ...
    const canje = canjeGuiaUSD(r.kg_fact);
    const listExtras = extrasDeCourier(r.courier);
    const extrasMonto = listExtras.reduce((s, e) => s + parseComma(e.monto), 0);
    ...
    const rows = [
      ...
      ...listExtras.map((e) => [td(e.descripcion), td("1,000"), td(fmt2(parseComma(e.monto))), td(fmt2(parseComma(e.monto)))]),
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6">
        ...
        {tab === "Paquetes en bodega" && (
          <PaquetesBodega packages={packages} flights={flights} user={currentUser} onUpdate={onUpdatePackage} />
        )}

        {tab === "Armado de cajas" && (
          <ArmadoCajas packages={packages} flights={flights} setFlights={setFlights} onAssign={(id) => {}} />
        )}
