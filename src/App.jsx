/*  Europa Envíos – MVP (con capturador de errores + uuid seguro)
    - Evita pantalla blanca con ErrorBoundary + GlobalErrorCatcher
    - Formatos con coma (peso: 3 decimales; dinero: 2 decimales)
*/
...
/* ===== Capturador Global (errores fuera de React) ===== */
function GlobalErrorCatcher({ children }) {
  const [fatal, setFatal] = useState(null);
  useEffect(() => {
    const onErr = (msg, src, line, col, err) => {
      setFatal(err || new Error(String(msg)));
      return false;
    };
    const onRej = (e) => setFatal(e?.reason || new Error("Unhandled Promise rejection"));
  ...
  if (fatal) {
    return (
      <div style={{padding:24, fontFamily:"sans-serif"}}>
        <h2>Se produjo un error (global)</h2>
...
const Input = (props) => (
  <input
    {...props}
    className={"w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 ring-indigo-500 " + (props.className || "")}
  />
);
const Tabs = ({ tabs, current, onChange }) => (
  <div className="flex gap-2 flex-wrap mb-4">
    {tabs.map((t) => (
      <button key={t} onClick={() => onChange(t)} className={"px-3 py-2 rounded-xl text-sm " + (current === t ? "bg-indigo-600 text-white" : "bg-white border")}>
        {t}
      </button>
    ))}
  </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-2xl shadow p-6 w-full max-w-md">
        ...
        <button onClick={() => onLogin({ email, role, courier: role === "ADMIN" ? null : courier })}
                disabled={!canSubmit}
      ...
      <div className="overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {flights.length === 0 && (
              <tr><td colSpan={6} className="text-center text-gray-500 py-6">Aún no hay cargas.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function Reception({ currentUser, couriers, setCouriers, estados, setEstados, flights, onAdd }) {
  ...
  const [form, setForm] = useState({
    ...
    peso_real_txt: "", largo_txt: "", ancho_txt: "", alto_txt: "",
  ...
  useEffect(() => {
    ...
    setForm((f) => ({ ...f, codigo }));
  }, [flightId, form.courier]); // eslint-disable-line

  const allReq = () =>
    ["courier","estado","casilla","codigo","fecha","empresa","nombre","tracking","remitente","peso_real_txt","largo_txt","ancho_txt","alto_txt","desc","valor_txt"]
  ...
  const submit = () => {
    ...
    const p = {
      ...
      codigo: form.codigo, fecha: form.fecha, empresa_envio: form.empresa,
      nombre_apellido: form.nombre, tracking: form.tracking, remitente: form.remitente,
      peso_real: peso, largo: L, ancho: A, alto: H,
      descripcion: form.desc, valor_aerolinea: parseComma(form.valor_txt),
      peso_facturable: Number(fact.toFixed(3)), peso_volumetrico: Number(vol.toFixed(3)),
      exceso_volumen: Number(exc.toFixed(3)), foto: form.foto, estado_bodega: "En bodega",
    };
    onAdd(p);
  ...
  // Imprimir etiqueta: genero el SVG en la pestaña actual y lo inserto en la ventana de impresión
  const printLabel = () => {
    if (!(form.codigo && form.desc)) { alert("Completá al menos Código y Descripción."); return; }
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    JsBarcode(svg, form.codigo, { format: "CODE128", displayValue: false, height: 50 });
    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Field label="Nº de paquete (código)" required>
          <Input value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value.toUpperCase() })} placeholder="EE250905-BOSSBOX1" />
        </Field>
...
function PaquetesBodega({ packages, flights, user, onUpdate }) {
  ...
  const rows = packages
    ...
    .filter((p) =>
      (p.codigo + p.casilla + p.tracking + p.nombre_apellido + p.courier).toLowerCase().includes(q.toLowerCase())
    );
  ...
  function exportXLSX() {
    const header = [
      th("COURIER"), th("CÓDIGO"), th("CASILLA"), th("FECHA"), th("EMPRESA ENVIO"),
      th("NOMBRE Y APELLIDO"), th("TRACKING"), th("REMITENTE"), th("PESO REAL"),
      th("PESO FACTURABLE"), th("LARGO"), th("ANCHO"), th("ALTO"),
      th("PESO VOLUMÉTRICO"), th("EXCESO DE VOLUMEN"), th("DESCRIPCIÓN"), th("VALOR EUR")
    ];
    const body = rows.map((p) => [
      td(p.courier), td(p.codigo), td(p.casilla), td(p.fecha), td(p.empresa_envio), td(p.nombre_apellido),
      td(p.tracking), td(p.remitente), td(fmtPeso(p.peso_real)), td(fmtPeso(p.peso_facturable)),
      td(fmtPeso(p.largo)), td(fmtPeso(p.ancho)), td(fmtPeso(p.alto)), td(fmtPeso(p.peso_volumetrico)),
      td(fmtPeso(p.exceso_volumen)), td(p.descripcion), td(fmtMoney(p.valor_aerolinea)),
    ]);
...
function ArmadoCajas({ packages, flights, setFlights, onAssign }) {
  ...
  function addBox() {
    ...
    setFlights(flights.map((f) =>
      f.id !== flightId ? f : { ...f, cajas: [...f.cajas, { id: uuid(), codigo: boxCode, paquetes: [], peso: "", L: "", A: "", H: "" }] }
    ));
    setBoxCode("");
  }
  function updBox(field, val) {
    ...
    setFlights(flights.map((f) =>
      f.id !== flightId ? f : { ...f, cajas: f.cajas.map((c) => (c.id !== activeBoxId ? c : { ...c, [field]: val })) }
    ));
  }
  function assign() {
    ...
    setFlights(flights.map((f) =>
      f.id !== flightId ? f : { ...f, cajas: f.cajas.map((c) => (c.id !== activeBoxId ? c : { ...c, paquetes: [...c.paquetes, pkg.id] })) }
    ));
    onAssign(pkg.id);
    setScan("");
  }
  ...
  function move(pid, toId) {
    ...
    setFlights((prev) => prev.map((f) =>
      f.id !== flightId ? f : { ...f, cajas: f.cajas.map((c) => (c.id === activeBoxId ? { ...c, paquetes: c.paquetes.filter((x) => x !== pid) } : c)) }
    ));
    setFlights((prev) => prev.map((f) =>
      f.id !== flightId ? f : { ...f, cajas: f.cajas.map((c) => (c.id === toId ? { ...c, paquetes: [...c.paquetes, pid] } : c)) }
    ));
  }
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-3">
          ...
          {flight && flight.cajas.map((c) => {
            ...
            return (
              <div key={c.id} className={`border rounded-xl p-3 mb-3 ${activeBoxId === c.id ? "ring-2 ring-indigo-500" : ""}`}>
                    ...
                    return (
                      <li key={pid} className="flex items-center gap-2 py-1 border-b">
                        ...
                        <button className="text-red-600 text-xs" onClick={() =>
                          setFlights(flights.map((f) => f.id !== flightId ? f : { ...f, cajas: f.cajas.map((x) => x.id !== c.id ? x : { ...x, paquetes: x.paquetes.filter((z) => z !== pid) }) }))
                        }>Quitar</button>
        ...
        <div className="md:col-span-3 flex justify-end">
          <button onClick={exportBoxes} disabled={!flight} className="px-3 py-2 bg-emerald-600 text-white rounded-xl disabled:opacity-50">
            Exportar XLSX (cajas)
          </button>
        </div>
      </div>
...
function CargasEnviadas({ packages, flights }) {
  ...
  const resumen = useMemo(() => {
    ...
    return flight.cajas.map((c, i) => {
      const peso = parseComma(c.peso),
            L = parseComma(c.L), A = parseComma(c.A), H = parseComma(c.H);
  ...
  function exportTodo() {
    if (!flight) return;
    const headerP = [
      th("COURIER"), th("CÓDIGO"), th("CASILLA"), th("FECHA"), th("EMPRESA ENVIO"), th("NOMBRE Y APELLIDO"),
      th("TRACKING"), th("REMITENTE"), th("PESO REAL"), th("PESO FACTURABLE"), th("LARGO"), th("ANCHO"),
      th("ALTO"), th("PESO VOLUMÉTRICO"), th("EXCESO DE VOLUMEN"), th("DESCRIPCIÓN"), th("VALOR EUR"),
    ];
      ...
      {!flight ? (
        ...
        <>
          ...
          <div className="overflow-auto mb-6">
            <table className="min-w-full text-sm">
              <thead><tr className="bg-gray-50">
                {["Courier","Casilla","Código","Fecha","Nombre","Tracking","Peso real","Facturable","Volumétrico","Exceso","Valor (EUR)"].map((h) => <th key={h} className="text-left px-3 py-2">{h}</th>)}
              </tr></thead>
              <tbody>
          ...
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              <thead><tr className="bg-gray-50">
                {["Nº de Caja","Courier","Peso","Largo","Ancho","Alto","Peso volumétrico"].map((h) => <th key={h} className="text-left px-3 py-2">{h}</th>)}
              </tr></thead>
              <tbody>
                {resumen.map((r) => (
                ...
                <tr>
                  <td></td><td className="px-3 py-2 font-semibold">Totales</td>
...
function Proformas({ packages, flights, extras }) {
  ...
  const porCourier = useMemo(() => {
    ...
    flight.cajas.forEach((c) => c.paquetes.forEach((pid) => {
      ...
      a.bultos++; a.kg_real += p.peso_real; a.kg_fact += p.peso_facturable; a.kg_exc += p.exceso_volumen;
    }));
    return Array.from(m.values());
  }, [flight, packages]);

  const extrasDe = (c) => extras.filter((e) => e.flight_id === flightId && e.courier === c)
      ...
      {!flight ? (
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gray-50">
                {["Courier","Bultos","Kg real","Kg facturable","Kg exceso","TOTAL USD","XLSX"].map((h) => <th key={h} className="text-left px-3 py-2">{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {porCourier.map((r) => {
                const proc = r.kg_fact * T.proc, fr = r.kg_real * T.fleteReal, fe = r.kg_exc * T.fleteExc, desp = r.kg_fact * T.despacho;
                const canje = canjeGuiaUSD(r.kg_fact), extrasMonto = extrasDe(r.courier), com = 0.04 * (proc + fr + fe + extrasMonto);
...
function Extras({ flights, couriers, extras, setExtras }) {
  ...
  const add = () => {
    ...
    setExtras([...extras, { id: uuid(), flight_id: flightId, courier, descripcion: desc, monto, estado }]);
    setDesc(""); setMonto("");
  };
...
export default function App() {
  ...
  function addPackage(p) {
    if (packages.find((x) => x.codigo === p.codigo)) { alert("Ya existe ese código."); return; }
        ...
        {!user ? (
          ...
          <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
            ...
            <main className="max-w-7xl mx-auto px-4 py-6">
              ...
              {tab === "Recepción" && (
                <Reception currentUser={user} couriers={couriers} setCouriers={setCouriers} estados={estados} setEstados={setEstados} flights={flights} onAdd={addPackage} />
              )}
              {tab === "Paquetes en bodega" && (
                <PaquetesBodega packages={packages} flights={flights} user={user} onUpdate={updatePackage} />
              )}
              {tab === "Armado de cajas" && (
                <ArmadoCajas packages={packages} flights={flights} setFlights={setFlights} onAssign={assignToBox} />
              )}
              {tab === "Cargas enviadas" && (
