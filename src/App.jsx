...
const parseComma = (txt) => {
  ...
  const s = String(txt).trim().replace(/\./g, "").replace(",", "."); // quita puntos (miles) y usa coma como decimal
  const n = Number(s);
...
// XLSX helpers
const th = (txt) => ({ v: txt, t: "s", s: { font: { bold: true, color:{rgb:"FFFFFFFF"} }, fill:{ fgColor:{rgb:"FF1F2937"} }, alignment:{ horizontal:"center"}, border: _bd() }});
const td = (v)   => ({ v, t: "s", s: { border:_bd() }});
const _bd = () => ({ top:{style:"thin",color:{rgb:"FF9CA3AF"}}, bottom:{style:"thin",color:{rgb:"FF9CA3AF"}}, left:{style:"thin",color:{rgb:"FF9CA3AF"}}, right:{style:"thin",color:{rgb:"FF9CA3AF"}} });
...
const Input = (props) => <input {...props} className={"w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 ring-indigo-500 "+(props.className||"")} />;

const Tabs = ({tabs,current,onChange}) =>
  <div className="flex gap-2 flex-wrap mb-4">
    {tabs.map(t => <button key={t} onClick={()=>onChange(t)} className={"px-3 py-2 rounded-xl text-sm "+(current===t?"bg-indigo-600 text-white":"bg-white border")}>{t}</button>)}
  </div>;
...
function ManageList({label, items, setItems}){
  ...
  return <div className="bg-gray-50 rounded-xl p-2">
...
function CargasAdmin({ flights, setFlights }){
  ...
  const [awb,setAwb]=useState(""); const [fac,setFac]=useState("");

  function create(){
    if(!code) return;
    setFlights([{ id:crypto.randomUUID(), codigo:code, fecha_salida:date, estado:"En bodega", awb, factura_cacesa:fac, cajas:[] }, ...flights]);
    setCode(""); setAwb(""); setFac("");
  }
  function upd(id, field, value){
    setFlights(flights.map(f=> f.id===id?{...f,[field]:value}:f));
  }
  return <Section title="Gestión de cargas" right={
    <div className="flex gap-2">
    ...
    <div className="overflow-auto">
      <table className="min-w-full text-sm">
        <thead><tr className="bg-gray-50">
          {["Código","Fecha salida","Estado","AWB","Factura Cacesa","Cajas"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}
        </tr></thead>
        <tbody>
          {flights.map(f=>(
            <tr key={f.id} className="border-b">
              ...
              <td className="px-3 py-2">
                <select className="border rounded px-2 py-1" value={f.estado} onChange={e=>upd(f.id,"estado",e.target.value)}>{ESTADOS_CARGA.map(s=><option key={s}>{s}</option>)}</select>
              </td>
          ...
          {flights.length===0 && <tr><td colSpan={6} className="text-center text-gray-500 py-6">Aún no hay cargas.</td></tr>}
        </tbody>
      </table>
    </div>
  </Section>;
...
function Reception({ currentUser, couriers, setCouriers, estados, setEstados, flights, onAdd }){
  ...
  const [form,setForm]=useState({
    ...
    peso_real_txt:"", largo_txt:"", ancho_txt:"", alto_txt:"", desc:"", valor_txt:"0,00",
    foto:null
  });
  // Números
  const peso = parseComma(form.peso_real_txt), L=parseComma(form.largo_txt), A=parseComma(form.ancho_txt), H=parseComma(form.alto_txt);
  const fact = Math.max(0.200, peso || 0);
  ...
  useEffect(()=>{ // cuando cambian vuelo o courier, sugerimos siguiente código
  ...
  // Guardar paquete
  const allReq = () => ["courier","estado","casilla","codigo","fecha","empresa","nombre","tracking","remitente","peso_real_txt","largo_txt","ancho_txt","alto_txt","desc","valor_txt"]
  ...
  const submit = ()=>{
    ...
    const p = {
      id: crypto.randomUUID(), flight_id: flightId,
      courier: form.courier, estado: form.estado, casilla: form.casilla, codigo: form.codigo, fecha: form.fecha,
      empresa_envio: form.empresa, nombre_apellido: form.nombre, tracking: form.tracking, remitente: form.remitente,
      peso_real: peso, largo: L, ancho: A, alto: H, descripcion: form.desc, valor_aerolinea: parseComma(form.valor_txt),
    ...
    onAdd(p);
    setForm({...form, casilla:"", codigo:"", empresa:"", nombre:"", tracking:"", remitente:"", peso_real_txt:"", largo_txt:"", ancho_txt:"", alto_txt:"", desc:"", valor_txt:"0,00", foto:null});
  };

  // Foto
  const onPickPhoto = (e)=>{
    const file = e.target.files?.[0]; if(!file) return;
    const reader=new FileReader(); reader.onload=()=> setForm(f=>({...f,foto:reader.result})); reader.readAsDataURL(file);
  };

  // Imprimir etiqueta (código de barras + datos)
  const printLabel = ()=>{
    if(!(form.codigo && form.desc)) { alert("Completá al menos Código y Descripción."); return; }
  ...
  return <Section title="Recepción de paquete" right={<div className="flex items-center gap-2">
    ...
    {showMgr && <div className="grid md:grid-cols-2 gap-3 mb-4">
    ...
    <div className="grid md:grid-cols-3 gap-4">
      ...
      <Field label="Estado" required>
        <select className="w-full rounded-xl border px-3 py-2" value={form.estado} onChange={e=>setForm({...form, estado:e.target.value})}>
          <option value="">Seleccionar…</option>{estados.map(s=><option key={s}>{s}</option>)}
        </select>
      </Field>

      <Field label="Casilla" required><Input value={form.casilla} onChange={e=>setForm({...form, casilla:e.target.value})} /></Field>
      <Field label="Nº de paquete (código)" required><Input value={form.codigo} onChange={e=>setForm({...form, codigo:e.target.value.toUpperCase()})} placeholder="EE250905-BOSSBOX1" /></Field>
      ...
      <Field label="Largo (cm)" required><Input value={form.largo_txt} onChange={e=>setForm({...form, largo_txt:e.target.value})} placeholder="50,0" /></Field>
      <Field label="Ancho (cm)" required><Input value={form.ancho_txt} onChange={e=>setForm({...form, ancho_txt:e.target.value})} placeholder="30,0" /></Field>
      <Field label="Alto (cm)" required><Input value={form.alto_txt} onChange={e=>setForm({...form, alto_txt:e.target.value})} placeholder="20,0" /></Field>
      ...
      <Field label="Foto del paquete"><input type="file" accept="image/*" capture="environment" onChange={onPickPhoto} /></Field>
    </div>
...
function PaquetesBodega({ packages, flights, user, onUpdate }){
  ...
  const rows = packages.filter(p => vueloEsBodega(p) && (user.role!=="COURIER" || p.courier===user.courier) )
  ...
  const start = (p)=>{ setEditing(p.id); setForm({
    ...p,
    peso_real_txt: fmtPeso(p.peso_real), largo_txt: fmtPeso(p.largo), ancho_txt: fmtPeso(p.ancho), alto_txt: fmtPeso(p.alto),
    valor_txt: fmtMoney(p.valor_aerolinea)
  }); };
  const save = ()=>{
    const upd = {
      ...form,
      peso_real: parseComma(form.peso_real_txt), largo: parseComma(form.largo_txt), ancho: parseComma(form.ancho_txt), alto: parseComma(form.alto_txt),
      valor_aerolinea: parseComma(form.valor_txt),
    };
    onUpdate(upd); setEditing(null);
  };

  function exportXLSX(){
    const header = [th("COURIER"), th("CÓDIGO"), th("CASILLA"), th("FECHA"), th("EMPRESA ENVIO"), th("NOMBRE Y APELLIDO"), th("TRACKING"), th("REMITENTE"), th("PESO REAL"), th("PESO FACTURABLE"), th("LARGO"), th("ANCHO"), th("ALTO"), th("PESO VOLUMÉTRICO"), th("EXCESO DE VOLUMEN"), th("DESCRIPCIÓN"), th("VALOR EUR")];
    const body = rows.map(p => [
      td(p.courier), td(p.codigo), td(p.casilla), td(p.fecha), td(p.empresa_envio), td(p.nombre_apellido), td(p.tracking), td(p.remitente),
      td(fmtPeso(p.peso_real)), td(fmtPeso(p.peso_facturable)), td(fmtPeso(p.largo)), td(fmtPeso(p.ancho)), td(fmtPeso(p.alto)), td(fmtPeso(p.peso_volumetrico)), td(fmtPeso(p.exceso_volumen)), td(p.descripcion), td(fmtMoney(p.valor_aerolinea))
    ]);
  ...
  return <Section title="Paquetes en bodega" right={<div className="flex gap-2">
    ...
    <div className="overflow-auto">
      <table className="min-w-full text-sm">
        <thead><tr className="bg-gray-50">
          {["Courier","Casilla","Código","Fecha","Nombre","Tracking","Peso real","Facturable","Volumétrico","Exceso","Valor (EUR)","Acciones"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}
        </tr></thead>
        <tbody>
          {rows.map(p => editing===p.id ? (
            <tr key={p.id} className="border-b">
              <td className="px-3 py-2"><Input value={form.courier} onChange={e=>setForm({...form, courier:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.casilla} onChange={e=>setForm({...form, casilla:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.codigo} onChange={e=>setForm({...form, codigo:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.fecha} onChange={e=>setForm({...form, fecha:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.nombre_apellido} onChange={e=>setForm({...form, nombre_apellido:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.tracking} onChange={e=>setForm({...form, tracking:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.peso_real_txt} onChange={e=>setForm({...form, peso_real_txt:e.target.value})} /></td>
              <td className="px-3 py-2"><Input value={form.peso_facturable} onChange={e=>setForm({...form, peso_facturable:parseComma(e.target.value)})} /></td>
              <td className="px-3 py-2"><Input value={form.peso_volumetrico} onChange={e=>setForm({...form, peso_volumetrico:parseComma(e.target.value)})} /></td>
              <td className="px-3 py-2"><Input value={form.exceso_volumen} onChange={e=>setForm({...form, exceso_volumen:parseComma(e.target.value)})} /></td>
              <td className="px-3 py-2"><Input value={form.valor_txt} onChange={e=>setForm({...form, valor_txt:e.target.value})} /></td>
              <td className="px-3 py-2">
                <button className="px-2 py-1 border rounded mr-2" onClick={save}>Guardar</button>
                <button className="px-2 py-1 border rounded" onClick={()=>setEditing(null)}>Cancelar</button>
              </td>
            </tr>
          ) : (
            <tr key={p.id} className="border-b">
    ...
    <div className="grid md:grid-cols-2 gap-6 mt-6">
      {["kg_real","kg_exceso"].map((key,ix)=>{
        const agg = {}; rows.forEach(p=>{ agg[p.courier]??={courier:p.courier,kg_real:0,kg_exceso:0}; agg[p.courier][key]+= key==="kg_real"?p.peso_real:p.exceso_volumen; });
        const data = Object.values(agg); const total = data.reduce((s,a)=>s+a[key],0);
        return <div key={key} className="bg-gray-50 rounded-xl p-3">
          <div className="text-sm text-gray-600 mb-2">{key==="kg_real"?"Kg reales":"Exceso volumétrico"} por courier. Total: {fmtPeso(total)} kg</div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data} dataKey={key} nameKey="courier" outerRadius={100} label>
                  {data.map((_,i)=><Cell key={i} fill={COLORS[(i+(ix?3:0))%COLORS.length]} />)}
                </Pie>
                <Tooltip/><Legend/>
              </PieChart>
            </ResponsiveContainer>
...
function ArmadoCajas({ packages, flights, setFlights, onAssign }){
  ...
  const [boxCode,setBoxCode]=useState(""); const [activeBoxId,setActiveBoxId]=useState(null); const [scan,setScan]=useState("");

  function addBox(){
    if(!flightId||!boxCode) return;
    setFlights(flights.map(f=> f.id!==flightId?f:{...f, cajas:[...f.cajas, {id:crypto.randomUUID(), codigo:boxCode, paquetes:[], peso:"", L:"", A:"", H:""}]}));
    setBoxCode("");
  }
  ...
  function exportBoxes(){
    ...
    flight.cajas.forEach((caja, idx)=>{
      ...
      const headers = Object.keys(byCourier); const max = headers.reduce((m,k)=>Math.max(m,byCourier[k].length),0);
      const rows = [];
      rows.push([td(""),td("")]);
  ...
  return <Section title="Armado de cajas">
    <div className="grid md:grid-cols-3 gap-4">
      ...
      <Field label="Caja activa">
        <select className="w-full rounded-xl border px-3 py-2" value={activeBoxId||""} onChange={e=>setActiveBoxId(e.target.value)}>
          <option value="">—</option>{flight?.cajas.map(c=> <option key={c.id} value={c.id}>{c.codigo}</option>)}
        </select>
      </Field>

      <Field label="Escanear / ingresar código de paquete">
        <Input value={scan} onChange={e=>setScan(e.target.value.toUpperCase())} onKeyDown={e=> e.key==="Enter" && assign()} placeholder="EE250905-BOSSBOX1" />
      </Field>
      ...
      {activeBoxId && <>
        ...
        <div className="text-sm text-gray-600">Volumétrico caja (A×H×L ÷ 6000): {fmtPeso(volCaja(flight.cajas.find(c=>c.id===activeBoxId)||{}))} kg</div>
      </>}
      ...
      <div className="md:col-span-3">
        ...
        {flight && flight.cajas.map(c=>{
          ...
          return <div key={c.id} className={`border rounded-xl p-3 mb-3 ${activeBoxId===c.id?"ring-2 ring-indigo-500":""}`}>
            <div className="flex items-center justify-between mb-2">
              <div className="font-medium">Caja {c.codigo} · <span className="text-xs text-gray-600">{etiqueta}</span></div>
            ...
            <ul className="text-sm max-h-48 overflow-auto">
              {c.paquetes.map(pid => {
                ...
                return <li key={pid} className="flex items-center gap-2 py-1 border-b">
                  ...
                  {flight.cajas.length>1 && <select className="text-xs border rounded px-1 py-0.5 ml-auto" defaultValue="" onChange={e=>move(pid, e.target.value)}>
...
function CargasEnviadas({ packages, flights }){
  ...
  const list = flights.filter(f=> f.estado!=="En bodega").filter(f=> !from || f.fecha_salida>=from).filter(f=> !to || f.fecha_salida<=to).filter(f=> !estado || f.estado===estado);
  const flight = flights.find(f=> f.id===flightId);

  const resumen = useMemo(()=>{
    if(!flight) return [];
    return flight.cajas.map((c, i)=>{
      const peso = parseComma(c.peso); const L=parseComma(c.L), A=parseComma(c.A), H=parseComma(c.H);
  ...
  function exportTodo(){
    if(!flight) return;
    // Hoja paquetes
    const headerP = [th("COURIER"), th("CÓDIGO"), th("CASILLA"), th("FECHA"), th("EMPRESA ENVIO"), th("NOMBRE Y APELLIDO"), th("TRACKING"), th("REMITENTE"), th("PESO REAL"), th("PESO FACTURABLE"), th("LARGO"), th("ANCHO"), th("ALTO"), th("PESO VOLUMÉTRICO"), th("EXCESO DE VOLUMEN"), th("DESCRIPCIÓN"), th("VALOR EUR")];
    const bodyP = packages.filter(p=>p.flight_id===flightId).map(p=>[
      td(p.courier), td(p.codigo), td(p.casilla), td(p.fecha), td(p.empresa_envio), td(p.nombre_apellido), td(p.tracking), td(p.remitente),
      td(fmtPeso(p.peso_real)), td(fmtPeso(p.peso_facturable)), td(fmtPeso(p.largo)), td(fmtPeso(p.ancho)), td(fmtPeso(p.alto)),
      td(fmtPeso(p.peso_volumetrico)), td(fmtPeso(p.exceso_volumen)), td(p.descripcion), td(fmtMoney(p.valor_aerolinea))
    ]);
    const {ws:wsP} = sheetFromAOAStyled("Paquetes", [headerP, ...bodyP]);

    // Hoja cajas
    const headerC = [th("Nº de Caja"), th("Courier"), th("Peso"), th("Largo"), th("Ancho"), th("Alto"), th("Peso volumétrico")];
    const bodyC = resumen.map(r=> [td(r.n), td(r.courier), td(fmtPeso(r.peso)), td(fmtPeso(r.L)), td(fmtPeso(r.A)), td(fmtPeso(r.H)), td(fmtPeso(r.vol))]);
    bodyC.push([td(""),td(""),td(fmtPeso(totPeso)),td(""),td(""),td(""),td(fmtPeso(totVol))]);
    const {ws:wsC} = sheetFromAOAStyled("Cajas", [headerC, ...bodyC]);

    downloadXLSX(`Detalle_${flight.codigo}.xlsx`, [{name:"Paquetes", ws:wsP}, {name:"Cajas", ws:wsC}]);
  }
  ...
  return <Section title="Cargas enviadas (En tránsito / Arribado)">
    <div className="grid md:grid-cols-5 gap-3">
      ...
      <div className="flex items-end"><button onClick={exportTodo} disabled={!flight} className="px-3 py-2 bg-emerald-600 text-white rounded-xl disabled:opacity-50 w-full">Exportar XLSX</button></div>
    </div>
    ...
    {!flight ? <div className="text-gray-500 mt-4">Elegí una carga para ver contenido.</div> :
      <>
        ...
        <div className="overflow-auto mb-6">
          <table className="min-w-full text-sm">
            <thead><tr className="bg-gray-50">{["Courier","Casilla","Código","Fecha","Nombre","Tracking","Peso real","Facturable","Volumétrico","Exceso","Valor (EUR)"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}</tr></thead>
            <tbody>
              {packages.filter(p=>p.flight_id===flightId).map(p=>(
                <tr key={p.id} className="border-b">
                  <td className="px-3 py-2">{p.courier}</td><td className="px-3 py-2">{p.casilla}</td><td className="px-3 py-2 font-mono">{p.codigo}</td><td className="px-3 py-2">{p.fecha}</td>
                  <td className="px-3 py-2">{p.nombre_apellido}</td><td className="px-3 py-2 font-mono">{p.tracking}</td>
                  <td className="px-3 py-2">{fmtPeso(p.peso_real)}</td><td className="px-3 py-2">{fmtPeso(p.peso_facturable)}</td>
                  <td className="px-3 py-2">{fmtPeso(p.peso_volumetrico)}</td><td className="px-3 py-2">{fmtPeso(p.exceso_volumen)}</td>
                  <td className="px-3 py-2">{fmtMoney(p.valor_aerolinea)}</td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            <thead><tr className="bg-gray-50">{["Nº de Caja","Courier","Peso","Largo","Ancho","Alto","Peso volumétrico"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}</tr></thead>
            <tbody>
              {resumen.map(r=>(
                <tr key={r.n} className="border-b">
                  <td className="px-3 py-2">{r.n}</td><td className="px-3 py-2">{r.courier}</td>
                  <td className="px-3 py-2">{fmtPeso(r.peso)}</td><td className="px-3 py-2">{fmtPeso(r.L)}</td><td className="px-3 py-2">{fmtPeso(r.A)}</td><td className="px-3 py-2">{fmtPeso(r.H)}</td>
...
function Proformas({ packages, flights, extras }){
  ...
  const porCourier = useMemo(()=>{
    ...
    flight.cajas.forEach(c=> c.paquetes.forEach(pid=>{
      const p=packages.find(x=>x.id===pid); if(!p) return;
      if(!m.has(p.courier)) m.set(p.courier,{courier:p.courier, bultos:0, kg_real:0, kg_fact:0, kg_exc:0});
      const a=m.get(p.courier); a.bultos++; a.kg_real+=p.peso_real; a.kg_fact+=p.peso_facturable; a.kg_exc+=p.exceso_volumen;
    }));
    return Array.from(m.values());
  },[flight,packages]);

  const extrasDe = (c) => extras.filter(e=> e.flight_id===flightId && e.courier===c).reduce((s,e)=> s+parseComma(e.monto),0);

  function exportX(r){
    const proc=r.kg_fact*T.proc, fr=r.kg_real*T.fleteReal, fe=r.kg_exc*T.fleteExc, desp=r.kg_fact*T.despacho;
    const canje=canjeGuiaUSD(r.kg_fact); const extrasMonto=extrasDe(r.courier); const com=0.04*(proc+fr+fe+extrasMonto);
    const total=proc+fr+fe+desp+canje+extrasMonto+com;

    const rows = [
      [td("")],[td("Europa Envíos")],[td("LAMAQUINALOGISTICA, SOCIEDAD LIMITADA")],[td("N.I.F.: B56340656")],[td("CALLE ESTEBAN SALAZAR CHAPELA, NUM 20, PUERTA 87, NAVE 87")],[td("29004 MÁLAGA (ESPAÑA)")],[td("(34) 633 74 08 31")],[td("")],
      [th("Factura Proforma")],[td("Fecha: "+new Date().toISOString().slice(0,10))],[td("")],
  ...
  return <Section title="Proformas por courier" right={
    <select className="rounded-xl border px-3 py-2" value={flightId} onChange={e=>setFlightId(e.target.value)}>
      <option value="">Seleccionar carga…</option>{flights.map(f=><option key={f.id} value={f.id}>{f.codigo}</option>)}
    </select>
  }>
    {!flight ? <div className="text-gray-500">Seleccioná una carga.</div> :
      <div className="overflow-auto">
        <table className="min-w-full text-sm">
          <thead><tr className="bg-gray-50">{["Courier","Bultos","Kg real","Kg facturable","Kg exceso","TOTAL USD","XLSX"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}</tr></thead>
          <tbody>
            {porCourier.map(r=>{
              const proc=r.kg_fact*T.proc, fr=r.kg_real*T.fleteReal, fe=r.kg_exc*T.fleteExc, desp=r.kg_fact*T.despacho;
              const canje=canjeGuiaUSD(r.kg_fact); const extrasMonto=extrasDe(r.courier); const com=0.04*(proc+fr+fe+extrasMonto); const tot=proc+fr+fe+desp+canje+extrasMonto+com;
              return <tr key={r.courier} className="border-b">
                <td className="px-3 py-2">{r.courier}</td><td className="px-3 py-2">{r.bultos}</td>
                <td className="px-3 py-2">{fmtPeso(r.kg_real)}</td><td className="px-3 py-2">{fmtPeso(r.kg_fact)}</td><td className="px-3 py-2">{fmtPeso(r.kg_exc)}</td>
...
function Extras({ flights, couriers, extras, setExtras }){
  const [flightId,setFlightId]=useState(""); const [courier,setCourier]=useState(""); const [desc,setDesc]=useState(""); const [monto,setMonto]=useState(""); const [estado,setEstado]=useState("Pendiente");
  const add = ()=>{ if(!(flightId && courier && desc && monto)) return; setExtras([...extras,{ id:crypto.randomUUID(), flight_id:flightId, courier, descripcion:desc, monto, estado }]); setDesc(""); setMonto(""); };
  ...
  return <Section title="Trabajos extras">
    <div className="grid md:grid-cols-5 gap-2">
      ...
      <Field label="Estado de cobro"><select className="w-full rounded-xl border px-3 py-2" value={estado} onChange={e=>setEstado(e.target.value)}><option>Pendiente</option><option>Cobrado</option></select></Field>
    </div>
    <div className="flex justify-end mt-2"><button onClick={add} className="px-3 py-2 bg-indigo-600 text-white rounded-xl">Agregar</button></div>
    <div className="overflow-auto mt-4">
      <table className="min-w-full text-sm">
        <thead><tr className="bg-gray-50">{["Carga","Courier","Descripción","Monto (USD)","Estado"].map(h=><th key={h} className="text-left px-3 py-2">{h}</th>)}</tr></thead>
        <tbody>
          {filtered.map(e=> <tr key={e.id} className="border-b"><td className="px-3 py-2">{flights.find(f=>f.id===e.flight_id)?.codigo}</td><td className="px-3 py-2">{e.courier}</td><td className="px-3 py-2">{e.descripcion}</td><td className="px-3 py-2">{fmtMoney(parseComma(e.monto))}</td><td className="px-3 py-2">{e.estado}</td></tr>)}
          {filtered.length===0 && <tr><td colSpan={5} className="text-center text-gray-500 py-6">Sin extras.</td></tr>}
        </tbody>
      </table>
    </div>
  </Section>;
...
export default function App(){
  ...
  function addPackage(p){ const dup=packages.find(x=>x.codigo===p.codigo); if(dup){ alert("Ya existe ese código."); return; } setPackages([p, ...packages]); }
