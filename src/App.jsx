...
    - Nueva pestaña: Pendientes (para gestionar asignaciones y cambios de carga)
    - Sin Casilla: asignar a casilla y mandar a Pendientes
    - Paquetes en Bodega: esconder gráficos de courier con 0 kg de exceso volumétrico
*/
...
  // === MODIFICACIÓN: Añadir "Pendientes"
  return ["Recepción","Paquetes sin casilla","Paquetes en bodega","Armado de cajas","Cargas enviadas","Gestión de cargas","Proformas","Usuarios","Extras", "Pendientes"];
}
...
// === MODIFICACIÓN: PaquetesSinCasilla recibe `onAssignToCasilla` para notificar a la app principal
function PaquetesSinCasilla({ currentUser, items, setItems, onAssignToCasilla }){
...
  // === NUEVA FUNCIÓN: Mover a pendientes ===
  function assignToCasilla(r){
    setEditPending(r);
  }

  function handleAssign(){
    if(!editPending) return;
    const casilla = editPending.casilla.trim().toUpperCase();
    const codigo = editPending.codigo.trim().toUpperCase();
    if(!casilla || !codigo){
      alert("Debes ingresar la casilla y el código de paquete.");
      return;
    }
...
      id: uuid(),
      type: "assign_casilla",
      data: {...editPending, casilla, codigo},
      createdAt: new Date().toISOString()
    });
    // Eliminar de la lista "sin casilla"
    setItems(items.filter(x=>x.id!==editPending.id));
    setEditPending(null);
  }
...
                          {/* === NUEVO BOTÓN === */}
                          <button className="px-2 py-1 border rounded bg-indigo-600 text-white" onClick={()=>assignToCasilla(r)}>Asignar casilla</button>
                        </div>
                    </td>
                  )}
                  {!isAdmin && (
                    <td className="px-3 py-2">
                      {r.tracking ? "Con tracking" : "Sin tracking"}
                    </td>
                  )}
                </>
                )}
...
        </table>
      </div>
      {/* === NUEVO MODAL PARA ASIGNAR CASILLA === */}
      <Modal open={!!editPending} onClose={()=>setEditPending(null)} title="Asignar casilla">
        {editPending && (
          <div className="space-y-4">
            <div className="text-sm text-gray-700">
              Asignando casilla a: <b>{editPending.nombre}</b> (Paquete Nº {editPending.numero})
            </div>
            <Field label="Casilla" required>
              <Input value={editPending.casilla||""} onChange={e=>setEditPending({...editPending,casilla:limpiar(e.target.value)})}/>
            </Field>
            <Field label="Código de paquete" required>
              <Input value={editPending.codigo||""} onChange={e=>setEditPending({...editPending,codigo:limpiar(e.target.value)})}/>
            </Field>
            <div className="flex justify-end gap-2">
              <button className={BTN} onClick={()=>setEditPending(null)}>Cancelar</button>
              <button className={BTN_PRIMARY} onClick={handleAssign}>Asignar y enviar a Pendientes</button>
            </div>
          </div>
        )}
      </Modal>
...
function PaquetesBodega({packages, flights, user, onUpdate, onDelete, pendingTasks, setPendingTasks}){
  const [q,setQ]=useState("");
  const [flightId,setFlightId]=useState("");
...
  // === MODIFICACIÓN: Paquetes en bodega son los que tienen estado "En bodega" Y no están pendientes
  const baseRows = packages
    .filter(p => flights.find(f=>f.id===p.flight_id)?.estado==="En bodega")
    .filter(p => !pendingTasks.some(t=>t.type==="change_flight" && t.data.id === p.id))
...
  };
  
  // === NUEVA FUNCIÓN: Mover paquete a otra carga (crea tarea pendiente) ===
  function changeFlight(p){
    const newFlightId = window.prompt("Ingresa el ID de la nueva carga:");
    if(!newFlightId) return;
    const newFlight = flights.find(f=>f.id===newFlightId);
    if(!newFlight){ alert("Carga no encontrada."); return; }
    setPendingTasks(prev=>[...prev,{
      id:uuid(),
      type:"change_flight",
      data:{ id: p.id, oldFlightId: p.flight_id, newFlightId: newFlight.id, newFlightCode: newFlight.codigo },
      createdAt: new Date().toISOString()
    }]);
  }
...
                      {/* === NUEVO BOTÓN === */}
                      <button className="px-2 py-1 border rounded" onClick={()=>changeFlight(p)} disabled={user.role==="COURIER"}>Cambiar Carga</button>
                    </div>
                  </td>
                </tr>
              );
...
          // === MODIFICACIÓN: Filtrar couriers con 0 de exceso de volumen ===
          const dataExc  = Object.entries(aggExc).filter(([_, kg_exceso]) => kg_exceso > 0).map(([courier,kg_exceso])=>({courier,kg_exceso}));
...
function CargasEnviadas({packages, flights, user, pendingTasks}){
  const [from,setFrom]=useState("");
  const [to,setTo]=useState("");
...
  const flight = flights.find(f=>f.id===flightId);
  const pref = user.role==="COURIER" ? courierPrefix(user.courier) : null;
  // === MODIFICACIÓN: Paquetes en cargas enviadas deben no estar pendientes ===
...
  function exportPaquetesXLSX(){
    if(!flight){ alert("Seleccioná una carga."); return; }
    const header = ["Courier","Código","Casilla","Fecha","Nombre","Tracking","Peso real","Facturable","Volumétrico","Exceso"].map(th);
    const body = paquetesDeVuelo.map(p=>[
...
    ]);
    const { ws } = sheetFromAOAStyled(`Paquetes_${flight.codigo}`, [header, ...body], {
      cols:[{wch:16},{wch:16},{wch:10},{wch:12},{wch:22},{wch:18},{wch:14},{wch:14},{wch:14},{wch:14}],
      rows:[{hpt:24}]
    });
    downloadXLSX(`Cargas_enviadas_${flight.codigo}.xlsx`, [{name:`Paquetes_${flight.codigo}`.slice(0,31), ws}]);
  }
...
          <button onClick={exportPaquetesXLSX} disabled={!flight} className={"px-3 py-2 bg-gray-800 text-white rounded-xl disabled:opacity-50"}>
            Exportar XLSX
          </button>
        </div>
      </div>
...
/* ========== NUEVO COMPONENTE: Pendientes ========== */
function Pendientes({ pendingTasks, setPendingTasks, packages, setPackages, flights, setFlights }) {
...
    const taskType = editingTask.type;
    const taskId = editingTask.id;
    const newData = editingTask.data;
    const newCasilla = newData.casilla?.trim().toUpperCase();
    const newCodigo = newData.codigo?.trim().toUpperCase();
    const newFlight = flights.find(f => f.id === newData.newFlightId);

    if (taskType === "assign_casilla" && (!newCasilla || !newCodigo)) {
      alert("La casilla y el código no pueden estar vacíos.");
      return;
    }
    if (taskType === "change_flight" && !newFlight) {
      alert("La carga de destino no es válida.");
      return;
    }

    setPendingTasks(prev => prev.map(t => t.id === taskId ? editingTask : t));
    setEditModalOpen(false);
    setEditingTask(null);
  };

  const handleComplete = (task) => {
    const ok = window.confirm(`¿Marcar como realizada la tarea "${task.type.replaceAll("_", " ")}"?`);
    if (!ok) return;

    const updatedTask = { ...task, completedAt: new Date().toISOString() };
    setPendingTasks(prev => prev.filter(t => t.id !== task.id));

    if (task.type === "assign_casilla") {
      const pkgData = task.data;
      const newPackage = {
        id: uuid(),
        flight_id: null, // Sin carga al principio
        courier: pkgData.courier || "",
        estado: "Aéreo", // O un valor por defecto apropiado
        casilla: pkgData.casilla.trim().toUpperCase(),
        codigo: pkgData.codigo.trim().toUpperCase(),
        codigo_full: `${pkgData.codigo.trim().toUpperCase()}`,
        fecha: pkgData.fecha,
        empresa_envio: pkgData.tracking || "",
        nombre_apellido: pkgData.nombre,
        tracking: pkgData.tracking || "",
        remitente: "", // No hay remitente en sin-casilla
        peso_real: 0, // No hay peso en sin-casilla
        largo: 0, ancho: 0, alto: 0,
        descripcion: "",
        valor_aerolinea: 0,
        peso_facturable: 0, peso_volumetrico: 0, exceso_volumen: 0,
        foto: null,
        estado_bodega: "En bodega"
      };
      setPackages(prev => [...prev, newPackage]);
      alert(`Paquete ${newPackage.codigo} creado y movido a Paquetes en bodega.`);
    } else if (task.type === "change_flight") {
      const pkgId = task.data.id;
      const newFlightId = task.data.newFlightId;
      setPackages(prev => prev.map(p => p.id === pkgId ? { ...p, flight_id: newFlightId } : p));
      alert(`Paquete ${pkgId} ha sido movido a la nueva carga.`);
    }
  };

  const handleDelete = (id) => {
    const ok = window.confirm("¿Eliminar esta tarea pendiente?");
    if (ok) {
...
              <th className="text-left px-3 py-2">Tipo de Tarea</th>
...
                  <td className="px-3 py-2">{new Date(task.createdAt).toLocaleDateString()}</td>
                  <td className="px-3 py-2 font-semibold">
                    {task.type === "assign_casilla" ? "Asignar Casilla" : "Cambiar de Carga"}
                  </td>
                  <td className="px-3 py-2">
                    {task.type === "assign_casilla" ? 
                      `Paquete Nº ${task.data.numero}: asignar casilla ${task.data.casilla} con código ${task.data.codigo}` :
                      `Paquete ${task.data.id} cambia de carga a ${task.data.newFlightCode}`
                    }
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex gap-2">
                      <button className={BTN} onClick={() => handleEdit(task)}>Editar</button>
                      <button className={BTN_PRIMARY} onClick={() => handleComplete(task)}>Realizado</button>
                      <button className="px-2 py-1 border rounded text-red-600" onClick={() => handleDelete(task.id)}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="4" className="text-center text-gray-500 py-6">No hay tareas pendientes.</td>
              </tr>
            )}
          </tbody>
        </table>
...
      <Modal open={editModalOpen} onClose={() => setEditModalOpen(false)} title="Editar Tarea Pendiente">
        {editingTask && (
          <div className="space-y-4">
            {editingTask.type === "assign_casilla" ? (
              <>
                <Field label="Casilla" required>
                  <Input value={editingTask.data.casilla} onChange={e => setEditingTask(prev => ({ ...prev, data: { ...prev.data, casilla: limpiar(e.target.value) } }))} />
                </Field>
                <Field label="Código de paquete" required>
                  <Input value={editingTask.data.codigo} onChange={e => setEditingTask(prev => ({ ...prev, data: { ...prev.data, codigo: limpiar(e.target.value) } }))} />
                </Field>
              </>
            ) : (
              <>
                <Field label="Paquete">
                  <Input value={editingTask.data.id} disabled />
                </Field>
                <Field label="Nueva Carga">
                  <select value={editingTask.data.newFlightId} onChange={e => setEditingTask(prev => ({...prev, data: {...prev.data, newFlightId: e.target.value, newFlightCode: flights.find(f=>f.id===e.target.value)?.codigo || ""}}))}>
                    {flights.map(f => <option key={f.id} value={f.id}>{f.codigo}</option>)}
                  </select>
                </Field>
              </>
            )}
            <div className="flex justify-end gap-2">
              <button className={BTN} onClick={() => setEditModalOpen(false)}>Cancelar</button>
              <button className={BTN_PRIMARY} onClick={handleSaveEdit}>Guardar</button>
            </div>
          </div>
        )}
      </Modal>
...
        if(Array.isArray(arr)) setSinCasillaItems(arr);
      }
    }catch{}
  },[]);
  useEffect(()=>{
...
  // === NUEVO: Tareas pendientes (persistencia) ===
  const PENDING_KEY = "ee_pending_v1";
...
      if (raw) {
        const arr = JSON.parse(raw);
        if (Array.isArray(arr)) setPendingTasks(arr);
      }
    } catch {}
  }, []);
  useEffect(() => {
...
            onAssignToCasilla={(task)=>setPendingTasks(prev=>[...prev, task])}
          />
        )}
